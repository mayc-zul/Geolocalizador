...
void LCDInit(){
    ...
    CmdWrite8(LCD_FUNCTIONSET | LCD_4BITMODE | LCD_2LINE | LCD_5x8DOTS);     // 4 bit,2 lines,5*8 bots -> 0b00101000
    CmdWrite8(LCD_DISPLAYMOVE | LCD_DISPLAYON | LCD_CURSOROFF | LCD_BLINKOFF);// lcd on, blink off, cursor off -> 0b00001100
    CmdWrite8(LCD_ENTRYMODESET | LCD_ENTRYLEFT);                             // increment cursor, no display shift -> 0b00000110
    CmdWrite8(LCD_CLEARDISPLAY);                                             // clear screen
    sleep_ms(2);
...
void cursorOn(){
    CmdWrite8(LCD_DISPLAYMOVE | LCD_DISPLAYON | LCD_CURSOROFF | LCD_BLINKON);         // 0b00001101
}
// Función desactivar el cursor
void cursorOff(){
    CmdWrite8(LCD_DISPLAYMOVE | LCD_DISPLAYON | LCD_CURSOROFF | LCD_BLINKOFF);       //0b00001100
}
// Función Retornar a la primera posición de la memoria del LCD
void returnHome(){
    CmdWrite8(LCD_RETURNHOME);
}
// Función para hacer corrimiento a la izquierda
void shiftCursoLeft(){
    CmdWrite8(LCD_CURSORSHIFT);
}
// Función para hacer corrimiento a la derecha
void shiftCursoRight(){
    CmdWrite8(LCD_CURSORSHIFT | LCD_CURSORMOVE | LCD_MOVERIGHT);      //0b00010100
}
...
void setCursor(uint8_t x,  uint8_t y){
	...
	x-=1;
	y-=1;
    tmp = LCD_SETDDRAMADDR + logicForYaxis[y] + x;
	CmdWrite8(tmp);
}
// Función para hacer Scrolling a la izquierda
void scrollDisplayLeft(){
    CmdWrite8(LCD_CURSORSHIFT | LCD_CURSORMOVE | LCD_MOVELEFT);        //0b00011000
}
